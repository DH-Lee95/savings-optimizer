...
const DEFAULT_INCOMING_PATH = "data/incoming/finlife-products.json";
...
export function importFinlifeProducts({
  ...
  if (!incoming) throw new Error(`Incoming FinLife file not found: ${incomingPath}`);
...
function main() {
  try {
    const summary = importFinlifeProducts();
    console.log(
      `Imported FinLife feed: ${summary.importedCount} imported, ${summary.newCount} new, ${summary.changedCount} changed, ${summary.unchangedCount} unchanged`,
    );
  } catch (error) {
