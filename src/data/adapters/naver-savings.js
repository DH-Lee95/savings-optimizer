...
export function parseNaverEligibility(text) {
  const sourceText = cleanText(text);
  const regions = parseNaverEligibleRegions(sourceText);
  const eligibility = {
    flags: [],
    sourceText,
  };
  ...
  const isYouthFutureSavings = /청년미래적금/.test(sourceText);
  ...
  if (!isYouthFutureSavings && isNaverMilitaryEligibilityText(sourceText)) eligibility.flags.push("military");
  if (!isYouthFutureSavings && isNaverVulnerableGroupEligibilityText(sourceText)) eligibility.flags.push("vulnerableGroup");
  if (/장애인|장애\s*정도|장애\s*등록/.test(sourceText)) eligibility.flags.push("disability");
  if (/유공자|국가유공|독립유공/.test(sourceText)) eligibility.flags.push("merit");
  if (/실버|백세|고령/.test(sourceText)) eligibility.flags.push("senior");
  if (!isYouthFutureSavings && isNaverChildEligibilityText(sourceText)) eligibility.flags.push("child");
  if (/임신|출산/.test(sourceText)) eligibility.flags.push("pregnancyOrBirth");
  if (!isYouthFutureSavings && isNaverBusinessOwnerEligibilityText(sourceText)) eligibility.flags.push("businessOwner");
  if (!isYouthFutureSavings && isNaverSmallBusinessEmployeeEligibilityText(sourceText)) eligibility.flags.push("smallBusinessEmployee");
...
function parseNaverEligibleRegions(text) {
  return NAVER_REGION_PATTERNS
    .filter(([, pattern]) => pattern.test(text))
    .map(([region]) => region);
}

function isNaverMilitaryEligibilityText(text) {
...
function isNaverVulnerableGroupEligibilityText(text) {
  return /기초생활\s*수급|차상위|희망나눔|취약\s*계층|한부모/.test(text);
}

function isNaverChildEligibilityText(text) {
...
function isNaverBusinessOwnerEligibilityText(text) {
  return /사업자등록증|개인사업자\s*(?:전용|대상|고객|가입)|법인\s*(?:고객|사업자|전용|가입)/.test(text);
}

function isNaverSmallBusinessEmployeeEligibilityText(text) {
